...
def _reassign_bad_bins(classe, x, y):
    ...
    #
    # LP 09/04/15: The following line has been changed from Cappellari's to work properly
    #              with Python 2.6.9
    #
    # Original:
    # index = np.argmin((x[bad, np.newaxis] - xnode) ** 2 + (y[bad, np.newaxis] - ynode) ** 2, axis=1)
    #
    index = np.argmin(((np.expand_dims(x, axis=1))[bad, :] - xnode) ** 2 +
                      ((np.expand_dims(y, axis=1))[bad, :] - ynode) ** 2, axis=1)
    ...
    for it in range(1, xnode.size):  # Do at most xnode.size iterations
        ...
        for k in good:
            ...
            if wvt:
                ...
                scale[k] = np.sqrt(np.sum(index) / sn)  # Eq. (4) of Diehl & Statler (2006)

        diff = np.sum((xnode - xnodeOld) ** 2 + (ynode - ynodeOld) ** 2)

        if not quiet:  # pragma: no cover
            print('Iter: %4i  Diff: %.4g' % (it, float(diff)))
...
def voronoi_2d_binning(x, y, signal, noise, targetSN, cvt=True,
                       pixelsize=None, plot=False, quiet=True, wvt=False):
    """
    PURPOSE:
          Perform adaptive spatial binning of Integral-Field Spectroscopic
          (IFS) data to reach a chosen constant signal-to-noise ratio per bin.
    ...
    CALLING SEQUENCE:
        ...
        binNum, xBin, yBin, xBar, yBar, sn, nPixels, scale = \
            voronoi_2d_binning(x, y, signal, noise, targetSN,
                               plot=False, quiet=False, wvt=False,
                               cvt=True, pixelsize=None)
...
def displayBinnedMap(binNum, signal, x_sig=None, y_sig=None,
    ...
    Input: x_sig and y_sig are arrays n*n long. As input for Cappellari's routines
           It assumes that x_sig={ x[0],x[0],...,x[0],x[1],x[1],...x[1],...,x[n],...x[n] }
                           y_sig={ y[0],y[1],...,y[n],y[0],y[1],...y[n],...y[n] }
    """
    sign_binned = np.zeros(np.max(binNum) + 1)
    for i in range(0, np.max(binNum) + 1):
        s = np.where(binNum == i)
        sign_binned[i] = np.average(signal[s])

    if size is None:  # pragma: no cover
        if x_sig is not None:
            size = np.int(np.sqrt(len(x_sig)))
        else:
            raise ValueError("Set either size or x_sig for displayBinnedMap")

    if size != np.int(np.sqrt(len(binNum))):  # pragma: no cover
        raise ValueError("size must be equal to sqrt[len(binNum)]")

    sigBinnedMap = np.zeros((size, size))
    for i in range(0, size):
        for j in range(0, size):
            sigBinnedMap[i, j] = sign_binned[binNum[i * size + j]]
